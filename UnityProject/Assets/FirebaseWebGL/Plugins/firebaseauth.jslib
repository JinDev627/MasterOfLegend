mergeInto(LibraryManager.library, {
    ...
    SignInAnonymously: function (objectName, callback, fallback) {
        ...
        try {
            firebase.auth().signInAnonymously(firebase_auth).then(function (result) {
                unityInstance.Module.SendMessage(parsedObjectName, parsedCallback, JSON.stringify(result));
    ...
    CreateUserWithEmailAndPassword: function (email, password, objectName, callback, fallback) {
        ...
        try {
            ...
            firebase.auth().createUserWithEmailAndPassword(firebase_auth, parsedEmail, parsedPassword).then(function (unused) {
    ...
    SignInWithEmailAndPassword: function (email, password, objectName, callback, fallback) {
        ...
        try {
            ...
            firebase.auth().signInWithEmailAndPassword(firebase_auth, parsedEmail, parsedPassword).then(function (unused) {
    ...
    SignInWithGoogle: function (objectName, callback, fallback) {
        ...
        try {
            ...
            firebase.auth().signInWithRedirect(firebase_auth, provider).then(function (unused) {
    ...
    OnAuthStateChanged: function (objectName, onUserSignedIn, onUserSignedOut) {
        ...
        firebase.auth().onAuthStateChanged(firebase_auth, function(user) {
            if (user) {
